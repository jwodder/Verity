...
also specified, the tabulars are centered.  The B<-l> option overrides any
previous B<-p> or B<-t> options.
...
Output is in the form of centered TeX tables.  The B<-t> option overrides any
previous B<-l> or B<-p> options.
...
false, and C<< <-> >>, which creates a statement which is true if & only if
