...
extern enum {txtTbl, latexTbl} tblType;
symbol** vars;
int varno;
...
void printTbl(FILE* out) {
 ...
 if (statements->qty)
  tblType == latexTbl ? printLaTeXTbl(out) : printTxtTbl(out);
 fputc(10, out);
 free(vars);
}
...
void printLaTeXTbl(FILE* tex) {
 ...
 foreach(statements, ex) {
  fputs(" & $", tex); printLaTeXExp(ex->val, tex); fputc('$', tex);
 }
 fputs(" \\\\ \\hline\n", tex);
 do {
  for (i=0; i<varno; i++) {
...
void printLaTeXExp(expr* ex, FILE* tex) {
 ...
 switch (ex->oper) {
  ...
  case NOT: fputs("\\neg ", tex); printLaTeXExp(ex->args[0], tex); break;
  case AND: 
   printLaTeXExp(ex->args[0], tex); fputs(" \\wedge ", tex);
   printLaTeXExp(ex->args[1], tex); break;
  case OR:
   printLaTeXExp(ex->args[0], tex); fputs(" \\vee ", tex);
   printLaTeXExp(ex->args[1], tex); break;
  case XOR:
   printLaTeXExp(ex->args[0], tex); fputs(" \\dot{\\vee} ", tex);
   printLaTeXExp(ex->args[1], tex); break;
  case THEN:
   printLaTeXExp(ex->args[0], tex); fputs(" \\rightarrow ", tex);
   printLaTeXExp(ex->args[1], tex); break;
  case EQ:
   printLaTeXExp(ex->args[0], tex); fputs(" \\leftrightarrow ", tex);
   printLaTeXExp(ex->args[1], tex); break;
  case ':':
   fputc(ex->sym->c, tex); fputs("\\::\\:", tex);
   printLaTeXExp(ex->args[0], tex); break;
